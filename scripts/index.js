(function($, undefined) {
	...
	$(document).on(
			...
			function(e) {
				...
				if (typeof href !== "undefined" && hash !== ""
						&& href !== href.replace(hash, "")
						&& hash.search("/") !== -1) {
					// remove the hash from the link to allow normal loading of
					// the page.
	...
	$(document)
			.on(
					"pagebeforechange",
					function(e, f) {
						var hash = $.mobile.path.parseUrl(f.toPage).hash, hashEl, hashElInPage;
						...
						} else if (typeof f.toPage !== "object" && hash !== ""
								...
								.widget(
										"mobile.h2linker",
										{
											...
											_create : function() {
												...
												this
														._on(
																".jqm-open-quicklink-panel",
																{
																	"click" : function() {
																		$(
																				".ui-page-active .jqm-quicklink-panel")
																				.panel(
																						"open");
																		return false;
																	}
														...
														._on(
																document,
																{
																	"pagebeforechange" : function() {
																		...
																		this.element
																				.find(
																						".jqm-quicklink-panel .ui-btn-active")
																				.removeClass(
																						"ui-btn-active");
																	}
																});
...
if (location.protocol.substr(0, 4) === 'file'
	...
	// Start with links with only the trailing slash and that aren't external
	// links
	var fixLinks = function() {
		$("a[href$='/'], a[href='.'], a[href='..']").not("[rel='external']")
				.each(
						function() {
	...
	// Check to see if ajax can be used. This does a quick ajax request and
	// blocks the page until its done
	$
			.ajax({
				url : '.',
				async : false,
...
$(document)
		.on(
				...
				".jqm-demos",
				function(event) {
					var search, page = $(this), that = this, searchUrl = ($(this)
							.hasClass("jqm-home")) ? "_search/" : "../_search/", searchContents = $(
							".jqm-search ul.jqm-list").find(
							"li:not(.ui-collapsible)"), version = $.mobile.version
							|| "dev", words = version.split("-"), ver = words[0], str = words[1]
							|| "", text = ver;
					...
					$(document).on("panelopen", ".jqm-search-panel",
							function() {
					...
					$(".jqm-navmenu-link")
							.on(
									...
									function() {
										page
												.find(
														".jqm-navmenu-panel:not(.jqm-panel-page-nav)")
												.panel("open");
									});

					// Turn off autocomplete / correct for demos search
					$(this).find(".jqm-search input").attr("autocomplete",
							"off").attr("autocorrect", "off");
					...
					// Initalize search panel list and filter also remove
					// collapsibles
					$(this).find(".jqm-search ul.jqm-list")
							.html(searchContents).listview({
								inset : false,
								theme : null,
								dividerTheme : null,
								icon : false,
					...
					$(this).find(".jqm-search-results-wrap ul.jqm-list").html(
							searchContents).listview({
						inset : true,
						theme : null,
						dividerTheme : null,
						icon : false,
					...
					// Search results page get search query string and enter it
					// into filter then trigger keyup to filter
					if ($(event.target).hasClass("jqm-demos-search-results")) {
						search = $.mobile.path.parseUrl(window.location.href).search
								.split("=")[1];
						setTimeout(
								function() {
									e = $.Event("keyup");
									...
									$(that)
											.find(
													".jqm-content .jqm-search-results-wrap input")
											.val(search).trigger(e).trigger(
													"change");
								}, 0);
					}
				});
...
$(document).one(
		...
		".jqm-demos",
		function(event) {
			$(this).find(".jqm-search-results-list li, .jqm-search li").each(
					function() {
								...
								.widget(
										...
										$.mobile.listview,
										{
											...
											handleKeyUp : function(e) {
												var search, input = this.element
														.prev("form").find(
																"input");

												if (e.which === $.ui.keyCode.DOWN) {
													if (this.element
															.find("li.ui-btn-active").length === 0) {
														this.element
																.find(
																		"li:first")
																.toggleClass(
...
function attachPopupHandler(popup, sources) {
	popup
			.one(
					"popupbeforeposition",
					function() {
						var collapsibleSet = popup
								.find("[data-role='collapsibleset']"), collapsible, pre;

						$
								.each(
										sources,
										function(idx, options) {
											collapsible = $("<div data-role='collapsible' data-collapsed='true' data-theme='"
													+ options.theme
											...
											pre.append(options.data.replace(
													/</gmi, '&lt;'));
											collapsible
						...
						collapsibleSet.find("[data-role='collapsible']")
								.first().attr("data-collapsed", "false");
		...
		$.ajax(absUrl, {
			...
			dataType : "text"
				...
				function(data, textStatus, jqXHR) {
					text = data;
					// If there's a hash we assume this is an HTML document that
					// has a tag
					// inside whose ID is the hash
					if (hash) {
						text = $("<div></div>").append(
								$(data).find(hash).contents().clone()).html();
					}
				});
	}
...
$(document)
		.bind(
				...
				function(e, data) {
					var popup, sources;
					if (data.options && data.options.role === "popup"
							&& data.options.link) {
						...
						if (sources) {
							...
							popup.appendTo($.mobile.activePage).popup().bind(
...
function makeButton() {
	...
	a = document.createElement("a"), txt = document
			.createTextNode("View Source");
...
$.fn.viewSourceCode = function() {
	return $(this).each(
			function() {
				var button = makeButton(), self = $(this), snippetSource = self
						.parents(".ui-page,:jqmData(role='page')").add(
								$("head")), fixData = function(data) {
				...
				if (self.is("[data-demo-html]")) {
					if (self.attr("data-demo-html") === "true") {
						...
						data = $("<div></div>").append(
								$(self.attr("data-demo-html")).clone()).html();
					}
					sources.push({
						title : "HTML",
						theme : "c",
				...
				if (self.is("[data-demo-js]")) {
					data = getSnippet("script", self.attr("data-demo-js"),
							snippetSource);
					sources.push({
						title : "JS",
						theme : "e",
				...
				if (self.is("[data-demo-css]")) {
					data = getSnippet("style", self.attr("data-demo-css"),
							snippetSource);
					sources.push({
						title : "CSS",
						theme : "f",
		...
		.on(
				...
				function(e) {
					...
					$(e.target).delegate(
							...
							"expand",
							function() {
								$(this).parents(":mobile-popup").popup(
										"reposition", {
											positionTo : "window"
										});
							});
					...
					$(e.target)
							.delegate(
									...
									"popupbeforeposition",
									function() {
										// max height: screen height - tolerance
										// (2*30px) - 42px for each collapsible
										// heading
										var x = $(this).find(".ui-collapsible").length, maxHeight = $.mobile
												.getScreenHeight()
												- 60 - (x * 42);
										...
										// keep line numbers and code lines in
										// sync
										$(
												".ui-collapsible:not(.ui-collapsible-collapsed) .gutter",
												this).find(".line").css(
												"height", "");

										$(
												".ui-collapsible:not(.ui-collapsible-collapsed) .code",
												this)
												.find(".line")
												.each(
														function() {
															if ($(this)
																...
																var height = $(
																		this)
																		.height(), linenumber = ".number"
																		+ /number(\w+)/
																				.exec(this.className)[1], gutter = $(
																		this)
																		.parents(
																				"tr")
																		.find(
